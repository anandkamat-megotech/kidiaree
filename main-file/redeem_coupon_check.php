...
include './services/service_no_coupon_check.php';
...
$serviceNoCouponCheck = new ServiceNoCouponCheck();
$serviceRedeemCouponCheck = new ServiceRedeemCouponCheck();
...
//Check whether couponCode is empty or not
...
if(!$isCouponCodeNonEmpty){
    ...
    $check=$serviceNoCouponCheck->serve($db, $idUser, $idCourseSelected, $isIndia);

    echo response_ok($check[0], $check[1], $check[2], $check[3], $check[4], $check[5], $check[6], $check[7],$check[8], $check[9], time());
    return;
}

//Check whether couponCode is Singleuse or not

$rowIdSigleuseCoupon = getSingleValue($db, "SELECT id FROM CouponsMaster WHERE BINARY name = ?", [$couponCode]);

if ($rowIdSigleuseCoupon != '') {
 
     $idCampaign = getSingleValue($db, "SELECT idCampaign FROM CouponsMaster WHERE  BINARY name = ?", [$couponCode]);
  ...
  $id = getSingleValue($db, "SELECT id FROM CouponsMaster WHERE BINARY name = ?", [$couponCode]);
  ...
  if($id == ''){
   ...
   $idUser = getSingleValue($db, "SELECT idUser FROM CouponsMaster WHERE BINARY name = ?", [$couponCode]);
  ...
  $check=$serviceRedeemCouponCheck->serve($db, $couponCode, $idCampaign, $idUser, $idCourseSelected, $isIndia);
...
//Check whether couponCode is Multiuse or not

$rowIdMultiuseCoupon = getSingleValue($db, "SELECT id FROM CampaignsMaster WHERE BINARY name = ?", [$couponCode]);
if($rowIdMultiuseCoupon == ""){
    echo response_not_found(time());
    return;
}
$rowUsageType = getSingleValue($db, "SELECT usageType FROM CampaignsMaster WHERE BINARY name = ?", [$couponCode]);

if($rowUsageType == "Singleuse"){
    echo response_not_found(time());
    return;
}
...
if ($rowIdMultiuseCoupon != '') {
  ...
  //Check whether campaign is still valid or not
  $idCampaign = getSingleValue($db, "SELECT id FROM CampaignsMaster WHERE BINARY name = ?", [$couponCode]);
  ...
  //Check whether coupon code is present or not
  $id = getSingleValue($db, "SELECT id FROM CampaignsMaster WHERE BINARY name = ?", [$couponCode]);
