...
class DB {
    ...
    public function is_meeting_scheduled($meetingTimestamp, $zoom_id, $name) {
        $result = $this->db->query("SELECT id FROM MultipleLiveSessions WHERE meetingTimestamp = '$meetingTimestamp' AND zoom_id = '$zoom_id' AND meetingDescription = '$name'");
        if($result->num_rows) {
            return true;
        }
