...
function create_meeting($entityBody) {
    ...
    $idPrerecordedSession = $jsonData['idPrerecordedSession'];
    ...
    $meetingSchedule = $db->is_meeting_scheduled($timestamp,$zoom_id,$topic);
    if($meetingSchedule){
       echo $str = '{"code": 403, "timestamp": '.time().', "message": "meeting already scheduled" }';
       exit();
    }
    ...
    try {
        ...
        $sql1 = "Insert into MultipleLiveSessions (idPrerecordedSession, meetingDescription, meetingLink, dd, mm, yyyy, hh, mmm, meetingTimestamp, zoom_id) values ($idPrerecordedSession, '$topic', '$data->id', $day, $month, $year, $hour, $minute,'$timestamp','$zoom_id');";
        $db->create_data($sql1);
        $sql2 = "Insert into slotsavailble (date, slots, coursesId, zoom_id) values ('$dateSlots', '$slot', '$cid','$zoom_id');";
        $db->create_data($sql2);
